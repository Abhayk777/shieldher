 ...
 */
function decryptBuffer(buffer: Buffer, keyBase64: string, ivBase64: string): Buffer {
...
/**
 * Encrypts text using AES-256-GCM for storage in Supabase.
 * Returns { iv, ciphertext } where ciphertext includes the appended auth tag.
 */
function encryptData(data: any, keyBase64: string): { iv: string; ciphertext: string } {
...
export async function POST(request: NextRequest) {
  try {
    const { uploadId, language = 'English', masterKey } = await request.json();
    ...
    try {
      ...
      // 1. Fetch and (optionally) decrypt data for all uploads
      const imageParts = await Promise.all(fileUrls.map(async (fileUrl: string, index: number) => {
        ...
        // Ephemeral decryption if the vault key was provided
        if (masterKey && fileIVs[index]) {
          try {
            buffer = decryptBuffer(buffer, masterKey, fileIVs[index]);
          } catch (err) {
            console.error('Decryption failed for file:', index, err);
            // If decryption fails, we continue with raw data (might be a legacy non-encrypted file)
          }
        }
        ...
        if (!mimeType || mimeType.startsWith('application/')) {
          try {
            ...
            if (pathname.endsWith('.mp3') || pathname.endsWith('.enc')) {
               // If it's an .enc file, we try to deduce from history or default to image
               mimeType = pathname.includes('audio') ? 'audio/mp3' : 'image/png';
            }
          } catch {
            mimeType = 'image/png';
          }
    ...
    // Prepare results (encrypt if masterKey is present)
    let payload: any = {
      upload_id: uploadId,
    ...
    if (masterKey) {
      const encryptedSummary = encryptData(result.summary, masterKey);
      const encryptedFlags = encryptData(result.flags, masterKey);
      const encryptedDetails = encryptData(result.details, masterKey);

      payload.encrypted_summary = encryptedSummary.ciphertext;
      payload.encrypted_flags = encryptedFlags.ciphertext;
      payload.encrypted_details = encryptedDetails.ciphertext;
      payload.encryption_iv = encryptedSummary.iv; // Same IV can be used if they are different payloads
